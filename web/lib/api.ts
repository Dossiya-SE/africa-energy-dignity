...
async function request<T>(path: string): Promise<ApiResult<T>> {
  try {
    ...
    if (!response.ok) {
      return { data: null, error: `AED API returned ${response.status} for ${path}.` };
    }
...
export function getMapAssets(): Promise<ApiResult<MapLayerRecord[]>> {
  return request<MapLayerRecord[]>("/map-assets");
}
